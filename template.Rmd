...
expand_year <- function(date_str) {
  ...
  # We ran into the problem of the year column creation on either side of the 21st century wehn making the new column. This logic fixed our issue
...
yearly_summary <- lyrics_df %>%
  group_by(year) %>%
  summarise(total_repeats = sum(fuzzy_repeats, na.rm = TRUE))

fuzzy_gg <- ggplot(yearly_summary, aes(x = year, y = total_repeats)) +
  geom_line(color = "blue") +
  geom_point(color = "black") +
  labs(
    title = "Fuzzy repaets by Year: Fuzzy",
    x = "Year",
    y = "Fuzzy Repeats"
  ) +
  theme_minimal()
