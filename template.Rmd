...
#test section to see if get_clean_lyrics() works
song_url <- search_song(test_title2, test_artist2, genius_token)

if (!is.na(song_url)) {
  lyrics <- get_clean_lyrics(song_url)
  cat(substr(lyrics, 1, 5000))  # print first 5000 characters
} else {
...
# Genius API Token
genius_token <- "BoaVCPyQtW90wUPpm3lPVgm1b5AJGB0BIBvG1UO9dj473mlajOuyvJ1hrN0Dvw9G"  # Replace this with your actual token

# --- 1. Scrape Billboard Number One Songs ---
decade_urls <- c(
  "https://en.wikipedia.org/wiki/List_of_Billboard_Hot_100_number-one_singles_from_1958_to_1969",
  "https://en.wikipedia.org/wiki/List_of_Billboard_Hot_100_number-one_singles_of_the_1970s",
  "https://en.wikipedia.org/wiki/List_of_Billboard_Hot_100_number-one_singles_of_the_1980s",
...
process_batch <- function(data, start_row, end_row, token, output_file) {
  ...
  for (i in start_row:end_row) {
    ...
    lyrics <- if (!is.na(url)) tryCatch(get_lyrics(url), error = function(e) NA) else NA
    Sys.sleep(0.5)
