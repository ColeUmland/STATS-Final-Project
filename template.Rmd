...
#The entire section above may or may not need to be added, as it just shows that there is a clear disparity in testing for song title in the lyrics for repetitiveness due to the variance in how titles are used in the lyrics. This is shown through the examples of Blinding Lights and Shake it Off
#############################
...
#The smaller the max distance, the closer the matches need to be to count. A max_distance of 7 would allow for some typos but wouldn't be too fuzzy to count too much
count_fuzzy_title_repeats <- function(title, lyrics_text, max_distance = 7, ngram_window = 5) {
...
fuzzy_results <- processed %>%
  mutate(
    fuzzy_repeats = map2_int(single, lyrics, ~ tryCatch(
      count_fuzzy_title_repeats(.x, .y, max_distance = 7),
      error = function(e) NA
    ))
  )
...
process_song_fuzzy <- function(single, lyrics, max_distance = 7) {
...
fuzzy_results <- processed %>%
  ...
  mutate(
    fuzzy_repeats = map2_int(single, lyrics, ~ {
      ...
      # Handle NA or NULL lyrics
      if (is.null(.y) || all(is.na(.y))) {
        cat("Lyrics missing for", .x, "\n")
        return(NA)
      }

      # If it's a character vector of multiple lines, combine into one string
      lyrics_text <- if (length(.y) > 1) paste(.y, collapse = " ") else .y

      tryCatch(
        count_fuzzy_title_repeats(.x, lyrics_text, max_distance = 7),
        error = function(e) {
