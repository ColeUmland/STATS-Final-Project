...
#Section 4.1 Reading in the Data Frame
```{r}
...
count_fuzzy_title_repeats <- function(title, lyrics_text, max_distance = 4, ngram_window = 5) {
...
fuzzy_results <- lyrics_df %>%
  mutate(
    fuzzy_repeats = map2_int(single, lyrics, ~ tryCatch(
      count_fuzzy_title_repeats(.x, .y, max_distance = 4),
      error = function(e) NA
    ))
  )
...
process_song_fuzzy <- function(single, lyrics, max_distance = 4) {
