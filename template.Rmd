...
#This table is closer to its goal. The 2000s additions to the table are off and still trying to remove the rows with repeating values (ones displaying years)
library(rvest)
library(dplyr)
library(purrr)
library(stringr)
...
all_number_ones <- map_df(decade_urls, function(url) {
  ...
  if (any(str_detect(names(table), "single|song")) && any(str_detect(names(table), "artist"))) {
    table %>%
      select(contains("date"), contains("single"), contains("song"), contains("artist")) %>%
      rename_with(~ c("date", "title", "artist")[1:length(.)]) %>%
