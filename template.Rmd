...
process_batch_combined <- function(data, start_row, end_row, token, output_file) {
  for (i in start_row:end_row) {
    ...
    write_csv(result_row, output_file, append = file.exists(output_file))
  }
}
...
#Section 3.1 Reading in the Data Frame
```{r}
lyrics_df <- read.csv("testing.csv")
```
...
```{r}
get_unique_word_percentage_from_text <- function(lyrics) {
  if (is.na(lyrics) || lyrics == "") {
    return(tibble(
      total_words = NA_integer_,
      unique_words = NA_integer_,
      unique_percentage = NA_real_
    ))
  }
  ...
  if (total_words == 0) {
    return(tibble(total_words = 0, unique_words = 0, unique_percentage = NA_real_))
  }

  tibble(
    total_words = total_words,
    unique_words = unique_words,
    unique_percentage = round(unique_words / total_words, 4)
  )
}

lyrics_df <- lyrics_df %>%
  mutate(stats = map(lyrics, get_unique_word_percentage_from_text)) %>%
  unnest(stats)
```
...
```{r}
avg_unique_by_year <- lyrics_df %>%
  group_by(year) %>%
  summarise(avg_unique_percentage = mean(unique_percentage, na.rm = TRUE))

ggplot(avg_unique_by_year, aes(x = year, y = avg_unique_percentage)) +
  geom_line(color = "steelblue", size = .7) +
  geom_point(color = "darkred", size = 2) +
  labs(
    title = "Average Song Uniqueness by Year",
    x = "Year",
    y = "Average Unique Percentage"
  ) +
  theme_minimal()
```
...
avg_stem_by_year <- lyrics_df %>%
  group_by(year) %>%
  summarize(frequency_pct.x = 
              mean(frequency_pct.x, na.rm = TRUE))


stem_gg <- ggplot(avg_stem_by_year, mapping = aes(x = year, y = frequency_pct.x)) +
  geom_line(color = "steelblue", size = .7) +
  geom_point(color = "darkred", size = 2) +
  labs(
